...
def Preupload_location(instance, filename, **kwargs):
    file_path = 'preinstalation/{author_id}/{title}/{title}-{filename}'.format(
        author_id=str(instance.author.id), title=str(instance.title), filename=filename)
    return file_path


class PreInstalation(models.Model):
...
@receiver(post_delete, sender=PreInstalation)
...
pre_save.connect(pre_save_blog_post_receiever, sender=PreInstalation)


#post instalation
#
def Postupload_location(instance, filename, **kwargs):
    file_path = 'postinstalation/{author_id}/{title}/{title}-{filename}'.format(
        author_id=str(instance.author.id), title=str(instance.title), filename=filename)
    return file_path


class Postinstalation(models.Model):
    title = models.CharField(max_length=50, null=False, blank=True)
    description = models.TextField(max_length=5000, null=False, blank=True)
...
@receiver(post_delete, sender=Postinstalation)
def submission_delete(sender, instance, **kwargs):
    instance.image.delete(False)
...
pre_save.connect(pre_save_blog_post_receiever, sender=Postinstalation)
